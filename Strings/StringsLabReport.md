...
Findings (high level)
MainActivity: Minimal Kotlin wrapper that loads libchallenge.so (contains stringFromJNI) and writes a date key (UUU0133) into SharedPreferences name DAD4 via method KLOW(). Not useful for flag directly.
...
Native library libflag.so: Heavily obfuscated in reverse tools; however, when loaded into memory the flag string is present in plaintext within its memory region.

Result: By setting SharedPreferences correctly and calling the activity with the correct base64 payload, the native library loaded and the flag was found in memory.
...
Install the APK on a device/emulator with Frida-server running and device adb accessible.

Run Frida attaching to package:
...
Observe output. The decrypted secret and memory hexdump will appear. The flag recovered:
...
Avoid embedding secrets in code or libraries.
...
Don’t expose entry points unnecessarily.
...
Avoid storing secrets in plain native memory.
...
Use runtime integrity checks and anti-instrumentation defenses (balanced and not a silver bullet).
